...
// Tipitaka data lives in a SEPARATE file (tipitaka.db)
// so PocketBase re-indexing to Qdrant won't touch it

function tipitakaDB() {
  // Opens tipitaka.db from pb_data directory
  return $dbx.open("sqlite", $app.dataDir() + "/tipitaka.db")
}
...
routerAdd("GET", "/api/tipitaka/search", function(c) {
  try {
    ...
    db.newQuery("SELECT COUNT(*) AS total FROM tipitaka WHERE lang={:lang} AND content LIKE {:like}")
      .bind({ lang: lang, like: like })
      .one(countRow)

    var rows = arrayOf(new DynamicModel({ volume: 0, page: 0, items: "", excerpt: "" }))
    db.newQuery("SELECT volume, page, items, substr(content, max(1, instr(content, {:q}) - 80), 300) AS excerpt FROM tipitaka WHERE lang = {:lang} AND content LIKE {:like} ORDER BY volume ASC, page ASC LIMIT {:per} OFFSET {:off}")
...
routerAdd("GET", "/api/tipitaka/read", function(c) {
  try {
    ...
    try {
      db.newQuery("SELECT volume, page, items, content, header, footer FROM tipitaka WHERE lang={:lang} AND volume={:vol} AND page={:page} LIMIT 1")
        .bind({ lang: lang, vol: volume, page: page })
        .one(row)
    } catch(_) { return c.json(404, { error: "page not found" }) }

    var maxRow = new DynamicModel({ max_page: 0 })
    db.newQuery("SELECT MAX(page) AS max_page FROM tipitaka WHERE lang={:lang} AND volume={:vol}")
...
routerAdd("GET", "/api/tipitaka/compare", function(c) {
  try {
    ...
    for (var i = 0; i < langs.length; i++) {
      ...
      try {
        db.newQuery("SELECT content, items, header, footer FROM tipitaka WHERE lang={:lang} AND volume={:vol} AND page={:page} LIMIT 1")
...
routerAdd("GET", "/api/tipitaka/volumes", function(c) {
  try {
    ...
    db.newQuery("SELECT volume, MAX(page) AS max_page FROM tipitaka WHERE lang={:lang} GROUP BY volume ORDER BY volume")
      .bind({ lang: lang })
      .all(rows)
