#!/usr/bin/env python3
"""
Import Tipitaka data into a SEPARATE tipitaka.db file
(NOT PocketBase's data.db — avoids Qdrant re-indexing)
"""
...
# Separate file — PocketBase re-indexer won't touch this
TIPITAKA_DB = os.path.join(PB_DATA, "tipitaka.db")
SRC_DIR = os.path.join(BACKEND, "tipitaka_dbs")

os.makedirs(PB_DATA, exist_ok=True)

conn = sqlite3.connect(TIPITAKA_DB)
...
if count > 0:
    print(f"Already imported ({count:,} rows) in tipitaka.db. Skipping.")
    conn.close()
    sys.exit(0)

def import_lang(lang, table, cols, build_row):
...
print("Importing Tipitaka into tipitaka.db (separate from data.db)...")
...
conn.close()
print(f"\nDone! {total:,} rows in tipitaka.db")
print(f"Location: {TIPITAKA_DB}")
